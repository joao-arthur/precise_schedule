...
pub static CONCERT_CREATE_SCHEMA: LazyLock<Schema> = LazyLock::new(|| {
...
pub fn transform_to_event_create(model: ConcertCreateInput) -> EventCreateInput {
    EventCreateInput {
        name: model.name,
...
pub async fn event_concert_create<
...
mod tests {
    ...
    use super::{ConcertCreateInput, event_concert_create, transform_to_event_create};

    #[test]
    fn test_transform_to_event_create() {
        assert_eq!(
            transform_to_event_create(ConcertCreateInput {
    ...
    #[tokio::test]
    async fn event_concert_create_ok() {
        let session = Session {
            id: "a6edc906-2f9f-5fb2-a373-efac406f0ef2".into(),
            username: "username".into(),
        };
        assert_eq!(
            event_concert_create(
                &session,
