...
pub async fn user_delete<Repo: UserRepository>(repository: &Repo, id: String) -> Result<User, UserErr> {
...
mod tests {
    ...
    async fn user_delete_ok() {
        assert_eq!(user_delete(&UserRepositoryStub::of_user(user_stub()), user_stub().id).await, Ok(user_stub()));
    }

    #[tokio::test]
    async fn user_delete_db_err() {
        assert_eq!(user_delete(&UserRepositoryStub::of_db_err(), user_stub().id).await, Err(UserErr::DB(DBErr)));
    }

    #[tokio::test]
    async fn user_delete_user_id_not_found_err() {
        assert_eq!(user_delete(&UserRepositoryStub::default(), user_stub().id).await, Err(UserErr::UserIdNotFound(UserIdNotFoundErr)));
    }
}
