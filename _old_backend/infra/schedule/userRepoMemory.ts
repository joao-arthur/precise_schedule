...
export function userRepoMemory(): UserRepo {
    ...
    return {
        cCreate: (user): Promise<Result<void, RepoErr>> => {
            users.push(user);
            return Promise.resolve(ok(undefined));
        },
        cUpdate: (userToUpdate: User): Promise<Result<void, RepoErr>> => {
            const index = users.findIndex((user) => user.id === userToUpdate.id);
            users.splice(index, 1, userToUpdate);
            return Promise.resolve(ok(undefined));
        },
        cReadById: (id: User["id"]): Promise<Result<User | undefined, RepoErr>> => {
        ...
        ): Promise<Result<User | undefined, RepoErr>> => {
