...
pub fn value_from_json_value(value: serde_json::Value) -> Val {
    match value {
        ...
        serde_json::Value::Array(arr) => Val::Arr(arr.into_iter().map(value_from_json_value).collect()),
        serde_json::Value::Object(obj) => Val::Obj(obj.into_iter().map(|f| (f.0, value_from_json_value(f.1))).collect()),
    }
}
