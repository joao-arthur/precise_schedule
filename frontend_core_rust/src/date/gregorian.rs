...
impl Dt {
    pub fn from(y: u16, m: u8, d: u8) -> Self {
        Dt { y: Y(y), m: M::of_u8(m).unwrap(), d: D(d) }
    }
}
...
impl ops::Add<interval::M> for Dt {
    ...
    fn add(self, intv: interval::M) -> Dt {
        ...
        let d = cmp::min(self.d.0, days_in_month(Y(y), M::of_u8(m as u8 + 1).unwrap()).0);
...
impl ops::Add<interval::D> for Dt {
    ...
    fn add(self, intv: interval::D) -> Dt {
        ...
        let mut m_days = u32::from(days_in_month(Y(y), M::of_u8(m as u8 + 1).unwrap()).0);
        while d >= m_days {
            m += 1;
            d -= m_days;
            if m > 11 {
            ...
            m_days = u32::from(days_in_month(Y(y), M::of_u8(m as u8 + 1).unwrap()).0);
        }
...
impl ops::Sub<interval::D> for Dt {
    ...
    fn sub(self, intv: interval::D) -> Dt {
        ...
        while d < 0 {
            ...
            let m_days = i64::from(days_in_month(Y(y), M::of_u8(m as u8 + 1).unwrap()).0);
            d += m_days;
        }
        Dt::from(y, m as u8 + 1, d as u8 + 1)
    }
...
fn leap_years_between(begin: Y, end: Y) -> u16 {
    let mut acc: u16 = 0;
    let mut y = begin.0;
    let rest = y % 4;
    if rest != 0 {
...
pub fn intv_between(a: Dt, b: Dt) -> interval::Dt {
    ...
    let mut m = ((b.y.0 as i16 * 12 + b.m.to_u8() as i16)
        - (a.y.0 as i16 * 12 + a.m.to_u8() as i16)) as u16;
    if m > 0 && b.d.0 < a.d.0 {
        m -= 1;
    }
    let d: u32 = 100;
...
mod test {
    ...
    fn test_date_month_from_u8() {
        assert_eq!(M::of_u8(0), None);
        assert_eq!(M::of_u8(1), Some(M::Jan));
        assert_eq!(M::of_u8(2), Some(M::Feb));
        assert_eq!(M::of_u8(3), Some(M::Mar));
        assert_eq!(M::of_u8(4), Some(M::Apr));
        assert_eq!(M::of_u8(5), Some(M::May));
        assert_eq!(M::of_u8(6), Some(M::Jun));
        assert_eq!(M::of_u8(7), Some(M::Jul));
        assert_eq!(M::of_u8(8), Some(M::Aug));
        assert_eq!(M::of_u8(9), Some(M::Sep));
        assert_eq!(M::of_u8(10), Some(M::Oct));
        assert_eq!(M::of_u8(11), Some(M::Nov));
        assert_eq!(M::of_u8(12), Some(M::Dec));
        assert_eq!(M::of_u8(13), None);
        assert_eq!(M::of_u8(14), None);
    }
    ...
    fn test_days_in_year() {
        assert_eq!(days_in_year(Y(400)), 366);
        assert_eq!(days_in_year(Y(800)), 366);
        assert_eq!(days_in_year(Y(1200)), 366);
        assert_eq!(days_in_year(Y(1600)), 366);
        assert_eq!(days_in_year(Y(2000)), 366);
        assert_eq!(days_in_year(Y(2004)), 366);
        assert_eq!(days_in_year(Y(2008)), 366);
        assert_eq!(days_in_year(Y(2012)), 366);
        assert_eq!(days_in_year(Y(2016)), 366);
        assert_eq!(days_in_year(Y(2020)), 366);
        assert_eq!(days_in_year(Y(2024)), 366);
        assert_eq!(days_in_year(Y(1300)), 365);
        assert_eq!(days_in_year(Y(1400)), 365);
        assert_eq!(days_in_year(Y(1500)), 365);
        assert_eq!(days_in_year(Y(1700)), 365);
        assert_eq!(days_in_year(Y(1800)), 365);
        assert_eq!(days_in_year(Y(1900)), 365);
        assert_eq!(days_in_year(Y(1900)), 365);
        assert_eq!(days_in_year(Y(1901)), 365);
        assert_eq!(days_in_year(Y(1902)), 365);
        assert_eq!(days_in_year(Y(1903)), 365);
        assert_eq!(days_in_year(Y(2001)), 365);
        assert_eq!(days_in_year(Y(2002)), 365);
        assert_eq!(days_in_year(Y(2001)), 365);
        assert_eq!(days_in_year(Y(2002)), 365);
        assert_eq!(days_in_year(Y(2003)), 365);
    }
